<template>
  <div class="ec-pro-pagination">
    <span>Showing {{ wishlists.from }}-{{ wishlists.to }} of {{ wishlists.total }} item(s)</span>
    <div v-if="wishlists.links.length > 3">
        <div class="ec-pro-pagination">
            <template v-for="(link, p) in wishlists.links" :key="p">
                <div v-if="link.url === null" class="mr-1 mb-1 px-4 py-3 text-sm leading-4 text-gray-400 border rounded" v-html="link.label" />
                <inertia-link v-else class="mr-1 mb-1 px-4 py-3 text-sm leading-4 border rounded hover:bg-white focus:border-indigo-500 focus:text-indigo-500" :class="{ 'bg-blue-700 text-white': link.active }" :href="link.url" v-html="link.label" />
            </template>
        </div>
    </div>
  </div>
</template>
<script>
import {InertiaLink} from "@inertiajs/inertia-vue3";
    export default {
        components:{InertiaLink},
        props: {
            wishlists: Object|Array
        },
    }
</script>