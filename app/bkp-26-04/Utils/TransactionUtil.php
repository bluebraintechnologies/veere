...
class TransactionUtil extends Util
{
     ...
     * @return array
     */
    public function getPurchaseProductsCustom($business_id, $transaction_id)
    {
        $records = \App\Transaction::join('purchase_lines as pl', 'transactions.id', '=', 'pl.transaction_id')
            ->leftjoin('products as p', 'pl.product_id', '=', 'p.id')->where('transactions.id', $transaction_id)->select('pl.product_id')->get();
        $productsIds = $records->pluck('product_id');
        
        $products = PurchaseLine::select('purchase_lines.product_id as product_id', 'purchase_lines.quantity as quantity', 'products.name as product_name', 'purchase_lines.lot_number', 'purchase_lines.exp_date',  'purchase_lines.is_print as pl_print' )
    ...
    {
        $product = \App\Transaction::join('purchase_lines as pl', 'transactions.id', '=', 'pl.transaction_id')
            ->leftjoin('products as p', 'pl.product_id', '=', 'p.id')->where('transactions.id', $transaction_id)->select('pl.product_id')->first();
        $lastLabel = \App\Label::where('product_id',  $product->product_id)->count();
            //return $lastLabel; 440
        $products = Transaction::join('purchase_lines as pl', 'transactions.id', '=', 'pl.transaction_id')
                            ...
                            ->leftjoin('labels as l', 'pl.product_id', '=', 'l.product_id')
                            ->where('transactions.business_id', $business_id)
                            ->where('transactions.id', $transaction_id)
                            ->where('transactions.type', 'purchase')
                            ->select('p.id as product_id', 'p.name as product_name', 'v.id as variation_id', 'v.name as variation_name', 'pl.exp_date', 'pl.lot_number',
                                'p.sku as sku', DB::raw('vld.qty_available - '.$lastLabel.'  as quantity'))
                            ->get();
